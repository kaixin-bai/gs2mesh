...
from gs2mesh_utils.cleardepth_stereo_utils import ClearDepthStereo
...
def run_single_debug(args):
    TSDF_voxel_length = args.TSDF_voxel / 512  # 2 / 512 = 0.00390625
    colmap_dir = os.path.abspath(os.path.join(base_dir, 'data', args.dataset_name,
                                              args.colmap_name))  # '/data/hdd1/kb/MyProjects/gs2mesh/data/DTU/scan24'
    ...
    if not args.skip_video_extraction:  # True
        video_name = f'{args.colmap_name}.{args.video_extension}'
        extract_frames(os.path.join(colmap_dir, video_name), os.path.join(colmap_dir, 'images'),
    ...
    if not args.skip_colmap:  # skip_colmap = True
        run_colmap(colmap_dir,
                   use_gpu=True)  # If there's an error regarding SiftGPU not being supported, set use_gpu to False
    ...
    if False:  # skip_GS = False
        try:
            os.chdir(
                os.path.join(base_dir, 'third_party', 'gaussian-splatting'))  # os.chdir的作用是改变当前工作目录，所有想读路径的文件操作将基于此目录
            iterations_str = ' '.join([str(iteration) for iteration in args.GS_save_test_iterations])  # '7000 30000'
            """
            在此处运行3dgs训练
            """
            os.system(
                f"python train.py -s {colmap_dir} --port {args.GS_port} --model_path {os.path.join(base_dir, 'splatting_output', strings['splatting'], args.colmap_name)} --iterations {args.GS_iterations} --test_iterations {iterations_str} --save_iterations {iterations_str}{' --white_background' if args.GS_white_background else ''}")
            os.chdir(base_dir)
        except:
            os.chdir(base_dir)
            print("ERROR")
    ...
    renderer = Renderer(base_dir,
                        ...
                        # '/data/hdd1/kb/MyProjects/gs2mesh/output/DTU_nw_iterations30000_DLNR_Middlebury_baseline7_0p/scan24'
                        args,
                        dataset=strings['dataset'],
                        # 'DTU_nw_iterations30000_DLNR_Middlebury_baseline7_0p', 这个文件夹在output/里
                        splatting=strings['splatting'],  # 'DTU_nw_iterations30000'
                        experiment_name=strings['experiment_name'],
                        # 'DTU_nw_iterations30000_DLNR_Middlebury_baseline7_0p'
                        device=device)  # 'cuda'
    ...
    if not args.skip_masking:
        if args.dataset_name == 'custom':
            if args.masker_automask:
                GD_model, predictor, inference_state, images_dir = init_predictor(base_dir, renderer, args,
                                                                                  device=device)
                masker = Masker(GD_model, predictor, inference_state, images_dir, renderer, stereo, args,
                                image_number=0, visualize=False)
                masker.segment()
                args.TSDF_use_mask = True
            else:
                print("Automask must be enabled for masking in script mode. Skipping.")
            ...
            masks_dir = os.path.join(colmap_dir, 'mask')
            masks_files = sorted([f for f in os.listdir(masks_dir) if os.path.isfile(os.path.join(masks_dir, f))],
                                 key=lambda x: x)
            masks_files = [mask for mask in masks_files if mask[0] != '.']
            for i, mask_file in enumerate(tqdm(masks_files)):
                stereo_output_dir = renderer.render_folder_name(i)
...
def run_single_clearDepth(args):
    TSDF_voxel_length = args.TSDF_voxel / 512  # 2 / 512 = 0.00390625
    colmap_dir = os.path.abspath(os.path.join(base_dir, 'data', args.dataset_name,
                                              args.colmap_name))  # '/data/hdd1/kb/MyProjects/gs2mesh/data/DTU/scan24'
    ...
    stereo = ClearDepthStereo(base_dir, renderer, args, device=device)
...
def run_single_clearDepth_and_minisplatting2(args):
    TSDF_voxel_length = args.TSDF_voxel / 512  # 2 / 512 = 0.00390625
    colmap_dir = os.path.abspath(os.path.join(base_dir, 'data', args.dataset_name,
                                              args.colmap_name))  # '/data/hdd1/kb/MyProjects/gs2mesh/data/DTU/scan24'
    ...
    if not args.skip_colmap:  # skip_colmap = True
        run_colmap_DTU_EXTENSIONS(colmap_dir,
                                  use_gpu=True)  # If there's an error regarding SiftGPU not being supported, set use_gpu to False
    ...
    if not args.skip_GS:  # skip_GS = False
        try:
            ...
            # os.chdir(
            #     os.path.join(base_dir, 'third_party', 'gaussian-splatting'))  # os.chdir的作用是改变当前工作目录，所有想读路径的文件操作将基于此目录
            # command = f"python train.py -s {colmap_dir} --port {args.GS_port} --model_path {os.path.join(base_dir, 'splatting_output', strings['splatting'], args.colmap_name)} --iterations {args.GS_iterations} --test_iterations {iterations_str} --save_iterations {iterations_str}{' --white_background' if args.GS_white_background else ''}"

            """
            python train.py \
              -s /data/hdd1/kb/MyProjects/gs2mesh/data/DTU_EXTENSIONS/scan1 \
              --port 8081 \
              --model_path /data/hdd1/kb/MyProjects/gs2mesh/splatting_output/DTU_EXTENSIONS_nw_iterations30000/scan1 \
              --iterations 30000  \
              --test_iterations 7000 30000  \
              --save_iterations 7000 30000  \
              --white_background  \
              --imp_metric indoor  \
              --config_path ../config/fast
            """

            os.chdir(
                os.path.join(base_dir, 'third_party', 'mini-splatting2',
                             'msv2'))  # os.chdir的作用是改变当前工作目录，所有想读路径的文件操作将基于此目录
            # command = f"python train.py -s {colmap_dir} --port {args.GS_port} --model_path {os.path.join(base_dir, 'splatting_output', strings['splatting'], args.colmap_name)} --iterations {args.GS_iterations} --test_iterations {iterations_str} --save_iterations {iterations_str}{' --white_background' if args.GS_white_background else ''} --imp_metric indoor --config_path ../config/fast"
            command = f"python train.py -s {colmap_dir} --model_path {os.path.join(base_dir, 'splatting_output', strings['splatting'], args.colmap_name)} --iterations {args.GS_iterations} --test_iterations {iterations_str} --save_iterations {iterations_str}{' --white_background' if args.GS_white_background else ''} --imp_metric indoor --config_path ../config/fast"
            print(command)
            os.system(
                command
            )
    ...
    if not args.skip_masking:
        if args.dataset_name == 'custom':
            ...
            for i, mask_file in enumerate(tqdm(masks_files)):
                stereo_output_dir = renderer.render_folder_name(i)
                mask = plt.imread(os.path.join(masks_dir, mask_file))[:, :, 0]
                cx, cy = renderer.cameras[0]['left']['cx'], renderer.cameras[0]['left']['cy']
                H, W = mask.shape
                W2 = min((W - cx), cx)
                H2 = min((H - cy), cy)
                crop_box = (
                ...
                mask = mask[crop_box[2]:crop_box[3], crop_box[0]:crop_box[1]]  # 这里是因为去畸变之后原图大小也发生变化了
                plt.imsave(os.path.join(stereo_output_dir, 'left_mask.png'), mask)
                np.save(os.path.join(stereo_output_dir, 'left_mask.npy'), mask)

        elif args.dataset_name == 'DTU_EXTENSIONS':
            masks_dir = os.path.join(colmap_dir, 'mask')
            extract_dimensions = lambda file_path: tuple(map(int, [token for token in
                                                                   open(file_path).readline().split()
                                                                   if token.replace('.', '', 1).isdigit()
                                                                   ][2:4]))
            img_shape = extract_dimensions(os.path.join(colmap_dir, 'sparse', '0', 'cameras.txt'))

            camera_params = read_cameras_text(os.path.join(colmap_dir, 'sparse', '0', 'cameras.txt'))
            ...
            camera_params = [{'width': camera_params[i].width,
                              ...
                              'cy': camera_params[i].params[2 if camera_params[i].model == 'SIMPLE_RADIAL' else 3]} for
                             i in camera_params_indices]
            ...
            for i, mask_file in enumerate(tqdm(masks_files)):
                ...
                # )
                # mask = mask[crop_box[2]:crop_box[3], crop_box[0]:crop_box[1]]  # 这里是因为去畸变之后原图大小也发生变化了
                mask = mask[:camera_params[0]['height'], :camera_params[0]['width']]  # 虽然这么做不正确，但是至少能用了
                # =============
                plt.imsave(os.path.join(stereo_output_dir, 'left_mask.png'), mask)
                np.save(os.path.join(stereo_output_dir, 'left_mask.npy'), mask)
